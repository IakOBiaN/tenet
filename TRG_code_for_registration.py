...
def main(model, lattice, chi_number, mu_diap = (-10, 10, 0.1), energy = 0.0, step = 0.01):
	...
	pt.subplot(221)
	pt.title("Изотерма")
	pt.xlabel("Химический потенциал, кДж/моль")
	...
	pt.xlabel("Химический потенциал, кДж/моль")
	pt.ylabel("Теплоёмкость, кДж/(моль*К)")
...
if __name__ == "__main__":
	...
	if model != "Langmuir":
		...
		if choice == "1":
			...
			exit()
		temp = float(input("Введите температуру одной цифрой в градусах Кельвина.\nВещественные числа вводятся через точку.\n"))
		mu_start = float(input("Введите диапазон для химического потенциала в кДж/моль.\nОтдельно вводятся три цифры: начало, конец, шаг. \nВведите начало диапазона \n"))
		...
		if model == "0NN":
			energy = float(input("Введите энергию взаимодействия между частицами в кДж/моль. \n"))
